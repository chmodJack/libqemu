                         ...
                         MMUAccessType access_type, MemOp op)
{
    ...
    MemoryRegionSection* section = iotlb_to_section(cpu, iotlbentry->addr, iotlbentry->attrs);
    MemoryRegion* mr = section->mr;
    hwaddr mr_offset = (iotlbentry->addr & TARGET_PAGE_MASK) + addr;
    ...
    uint64_t val;
    MemTxResult r = memory_region_dispatch_read(mr, mr_offset, &val, op, iotlbentry->attrs);
    if (r != MEMTX_OK)
	{
                      ...
                      uintptr_t retaddr, MemOp op)
{
    ...
    MemTxResult r = memory_region_dispatch_write(mr, mr_offset, val, op, iotlbentry->attrs);
    if (r != MEMTX_OK)
	{
		hwaddr physaddr = mr_offset + section->offset_within_address_space - section->offset_within_region;
		cpu_transaction_failed(cpu, physaddr, addr, memop_size(op),	MMU_DATA_STORE, mmu_idx, iotlbentry->attrs, r, retaddr);
	}
