...
static void arm_setup_direct_kernel_boot(ARMCPU *cpu, struct arm_boot_info *info)
{
    ...
    kernel_size = arm_load_elf(info, &elf_entry, &image_low_addr, &image_high_addr, elf_machine, as);
    ...
    if (kernel_size < 0)
	{
        error_report("could not load kernel: %s, kernel_size: %d", info->kernel_filename,kernel_size);
        exit(1);
    }
